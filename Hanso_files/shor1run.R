shor1run <- function(fn,gr,x0, maxit = 1000,  fvalquit = -Inf,
  ...
  if(is.nainf(f)){
    ...
    return(x = c(x), f = f, g = g, B = B, frec = frec, fevalrecall = fevalrecall, 
  ...
  for(iter in 1:maxit){
    ...
    if(is.na(beta)){
      ...
      if(betasq > 1) betasq <- 1
      else if(betasq <0) betasq <- 0
      else if(is.na(betasq)) betasq <- 0.25
      
      beta <- sqrt(betasq)
    }
    betarec[iter] <- beta
    B <- B%*%(I+(beta-1)%*%xi%*%t(xi))
    if(rescale) B <- B/norm(B,"I")
  }
