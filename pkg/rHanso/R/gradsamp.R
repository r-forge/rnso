gradsamp <- function(fn, gr = NULL, nvar, x0 = NULL, upper = 1, lower = 0, f0 = fn(x0), g0 = gr(x0), samprad = c(1e-04, 
  ...
  if (!is.null(x0)) {
    ...
    if (class(x0) == "numeric") {
      ...
      nvar <- nrow(x0)
      nstart <- ncol(x0)
      # if(nstart>1) stop('gradient sampling does not support multiple starting point, please use only on
      # point')
  ...
  for (run in 1:nstart) {
    ...
    if ((is.na(f0)) || (f0 == Inf) || (maxit == 0) && (prtlevel > 0)) {
      ...
      tmp <- gradsamp1run(fn, gr, x0[, run], nvar, f0, g0, samprad, maxit, normtol, ngrad, fvalquit, 
        prtlevel)
      x[, run] <- tmp$x
      f[run] <- tmp$f
      g[, run] <- tmp$g
  ...
  return(list(x = x, f = f, g = g, dnorm = dnorm, X = X, G = G, w = w, message = mess))
} 
